...
def parse_arguments():
    ...
    parser.add_argument('--out_dim', default=128, type=int, help='Dimensionality of the DINO head output.')
    parser.add_argument('--teacher_temp', default=0.04, type=float, help='Final value of the teacher temperature.')  
    parser.add_argument('--global_crops_scale', type=float, default=(0.4, 1.), help='Scale range of the cropped image before resizing, relatively to the origin image.')
    parser.add_argument('--local_crops_number', type=int, default=4, help='Number of small local views to generate.')
    parser.add_argument('--local_crops_scale', type=float, default=(0.05, 0.4), help='Scale range of the cropped image before resizing, relatively to the origin image.')
    parser.add_argument('--warmup_teacher_temp_epochs', default=1, type=int, help='Number of warmup epochs for the teacher temperature (Default: 30).')    
    parser.add_argument('--warmup_teacher_temp', default=0.01, type=float, help='Initial value for the teacher temperature: 0.04 works well in most cases.')
    ...
    try:
        args.jobid = os.environ["PBS_JOBID"].replace(".lmbtorque.informatik.uni-freiburg.de", "")
    except KeyError:
        args.jobid = "local"

    args.exp_name += "_{}_{}".format(args.exp_suffix, args.jobid)

    args.output_folder = check_dir(os.path.join(args.output_root, args.exp_name))
...
def main(args):
    # ============ wandb ============
    wandb.init(project="cv_assignment",
               name=args.exp_name + time.strftime("%Y%m%d-%H%M%S"),
               config={
                     "lr": args.lr,
                     "bs": args.bs,
    ...
    # ============ preparing loss ... ============
    # raise NotImplementedError("TODO: load weight initialization")
    dino_loss = DINOLoss(out_dim=args.out_dim, ncrops=args.local_crops_number+2, warmup_teacher_temp=args.warmup_teacher_temp,
                         teacher_temp=args.teacher_temp, warmup_teacher_temp_epochs=args.warmup_teacher_temp_epochs,
                         nepochs=args.epochs)
    ...
    for epoch in range(args.epochs):
        ...
        train(train_loader, teacher, student, dino_loss, optimizer, lr_schedule, wd_schedule, momentum_schedule, epoch, fp16_scaler, log=log)
        # save model
        print('==> Saving...')
        state = {
            'opt': args,
...
def train(data_loader, t_model, s_model, criterion, optimizer, lr_schedule, wd_schedule, momentum_schedule, epoch, fp16_scaler, log=None):
    ...
    for iteration, images in enumerate(data_loader):
        ...
        #     raise NotImplementedError("TODO: load weight initialization")
        s_op = s_model(images)
        t_op = t_model(images[:2])
        loss = criterion(s_op, t_op, epoch)
        # student update
        optimizer.zero_grad()
        param_norms = None
        ...
        if iteration % 100 == 0:
            if "PBS_JOBID" not in os.environ:
                # print("Epoch:{}, Avg loss = {}".format(epoch, loss_meter.mean))
                # log metrics wandb
                wandb.log({"train_loss": loss_meter.mean})
                print('Epoch:[{0}][{1}/{2}] | Avg loss: ({loss:0.4f})'.format(epoch, iteration, len(data_loader), loss=loss_meter.mean))
...
def validate(loader, t_model, s_model, criterion, epoch, args, log=None):
    ...
    # log metrics wandb
    wandb.log({"val_loss": loss_meter.mean})

    log.add_scalar("validation_loss", loss_meter.mean, global_step)
    # raise NotImplementedError("TODO: load weight initialization")

    ## t-SNE (t-distributed stochastic neighbor embedding) is a visualization technique used
    # to visualize high-dimensional data in a lower-dimensional space, typically two dimensions.
    n_dimensions = 2
    feature_maps_embedded = TSNE(n_components=n_dimensions).fit_transform(feature_maps)
...
class DINOLoss(torch.nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch, train=False):
        ...
        self.center = self.center.to(student_out[0].device)

        # teacher centering and sharpening
        # raise NotImplementedError("TODO: load weight initialization")
        temp = self.teacher_temp_schedule[epoch]
        teacher_out = torch.nn.functional.softmax((teacher_output - self.center) / temp, dim=-1)
        ...
        for iq, q in enumerate(teacher_out):
            for v in range(len(student_out)):
                if v == iq:
                    ...
                    continue
                loss = torch.sum(-q * torch.nn.functional.log_softmax(student_out[v], dim=-1), dim=-1)
    ...
    def update_center(self, teacher_output):
        ...
        batch_center = torch.nn.functional.normalize(batch_center, dim=-1)
